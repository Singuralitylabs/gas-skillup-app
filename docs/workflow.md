...
**設計ドキュメント作成** (2日)
- [ ] 画面遷移図作成（受講生・運営それぞれ）
- [ ] UI/UX設計書作成（ワイヤーフレーム、デザインシステム）
- [ ] データベース設計書作成（ER図、テーブル定義）
- [ ] API仕様書作成（エンドポイント一覧）
- [ ] モックデータ仕様書作成

**フロントエンド基盤** (2日)
- [ ] プロジェクト構成設計（app router構造）
- [ ] 共通UIコンポーネント実装（Button, Input, Card, Badge等）
...
**モックデータ準備** (2日)
- [ ] モックデータ構造設計（ユーザー、コンテンツ、課題、進捗）
- [ ] モックデータ生成関数実装（lib/mock/）
- [ ] ローカルストレージでの状態管理（一時的）
- [ ] TypeScript型定義完成
...
- [ ] ログイン画面（UI のみ、モック認証）
- [ ] ホーム画面（進捗サマリー、お知らせ表示）
- [ ] コンテンツ一覧画面（Phase/Week階層表示）
- [ ] コンテンツ詳細画面（動画・テキスト・演習）
- [ ] 課題提出画面（フォーム実装）
- [ ] 課題履歴画面（提出済み一覧）
...
**運営ダッシュボード** (3日)
- [ ] ダッシュボード画面UI（統計サマリー表示）
- [ ] チャート・グラフコンポーネント（Chart.js or Recharts）
- [ ] KPIカード（受講生数、進捗率、課題提出数）
- [ ] フィルター・期間選択機能
