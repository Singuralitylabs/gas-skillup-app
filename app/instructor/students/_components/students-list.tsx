...
export function StudentsList({ students, stats }: Props) {
	...
	const filteredAndSortedStudents = useMemo(() => {
		...
		return sorted;
	}, [students, searchQuery, approvalFilter, sortKey, sortOrder]);
		...
		<div className="space-y-6">
			...
			<div className="grid gap-4 sm:grid-cols-3">
				<div className="rounded-lg border border-border p-4">
					...
					<p className="text-2xl font-bold">{stats.total}人</p>
				</div>
				<div className="rounded-lg border border-border p-4">
					<p className="text-sm text-muted-foreground">承認済み</p>
					<p className="text-2xl font-bold">{stats.approved}人</p>
				</div>
				<div className="rounded-lg border border-border p-4">
					<p className="text-sm text-muted-foreground">未承認</p>
					<p className="text-2xl font-bold">{stats.pending}人</p>
				</div>
			</div>
						...
						<tbody className="divide-y divide-border">
							{filteredAndSortedStudents.length > 0 ? (
								filteredAndSortedStudents.map((student) => (
									...
									>
										<td className="p-4">
											<p className="font-medium">{student.name || "未設定"}</p>
										</td>
										<td className="p-4">
